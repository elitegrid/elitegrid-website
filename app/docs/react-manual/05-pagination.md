...
> **Pagination vs. virtualization:** EliteGrid can render a million rows smoothly *without* pagination thanks to virtualization (see [Chapter 14](/docs/react/performance)). Pagination is a UX choice, not a performance requirement.
...
- Your data comes from a server one page at a time (see [Chapter 14](/docs/react/performance#server-side-data-with-datasource)).
...
| Custom pager shows a stale page number | Reading `getPaginationState()` once instead of on every `onPageChange` | Re-read the state inside the event handler, as in the example above |
...
Next: [06 · Row Selection](/docs/react/selection)
