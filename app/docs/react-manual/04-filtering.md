...
| `filterIcon` | `ReactNode` | — | Replace the default funnel icon |
| `activeFilterIcon` | `ReactNode` | — | Icon shown when a filter is active |
...
Use your own icons (any React node — an SVG, an emoji, a component):

```ts
filtering: {
  filterIcon: <span>⏷</span>,           // normal state
  activeFilterIcon: <span>🔍</span>,     // when a filter is applied
...
If a filter returns zero rows, the grid shows a "No matching results" empty state. You can customize that message — see [Chapter 09 · Appearance](/docs/react/appearance#empty-states).
...
Combine this with `onFilterChange` (writing the model back to the URL or `localStorage`) for the same "restore on reload" pattern used for sorting in [Chapter 03](/docs/react/sorting#persisting-sort-across-page-reloads).
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Typing feels laggy on a large dataset | `debounceMs` too low, or a heavy `customFilter` running on every keystroke | Raise `debounceMs`, or move filtering to the server with a `dataSource` (see [Chapter 14](/docs/react/performance#server-side-data-with-datasource)) |
...
Next: [05 · Pagination](/docs/react/pagination)
