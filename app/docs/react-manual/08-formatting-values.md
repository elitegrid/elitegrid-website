...
`display` also has a `cellClass` option for conditional styling (colour-coding a status, highlighting an outlier value) — covered in [Chapter 02](/docs/react/columns#conditional-cell-styling-with-cellclass). It's independent of `formatter`, so the two are commonly used together on the same column: `formatter` decides *what text* is shown, `cellClass` decides *how it looks*.
...
Next: [09 · Appearance & Theming](/docs/react/appearance)
