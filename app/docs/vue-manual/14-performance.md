...
You do **not** need pagination for performance — virtualization already handles huge datasets. Use pagination only if page-by-page navigation is the UX you want. See [Chapter 05](/docs/vue/pagination).
...
> **`async` and `Promise` (used below).** Fetching from a server takes time, so `getRows` is marked `async` and returns a `Promise` — a placeholder for "a value that will arrive later". The `await` keyword pauses inside the function until the network response comes back. The grid shows its loading overlay (see [Chapter 09](/docs/vue/appearance#loading-state)) while it waits.
...
- ✅ Create the grid **once**, directly in `<script setup>` — not inside a `computed` or `watchEffect` that reruns (see [Ch. 01](/docs/vue/getting-started)).
...
- **Vue DevTools → Component inspector / Performance tab.** Record an interaction (a sort, a filter keystroke) and check which components re-rendered. If components far outside the grid re-render on every keystroke, look for reactive state that's broader than it needs to be (e.g. a shared `reactive()` object more components depend on than necessary).
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Typing in a filter box feels laggy | `debounceMs` too low, or a heavy `customFilter`/`formatter` | Raise `debounceMs` (see [Chapter 04](/docs/vue/filtering)); keep formatters and custom filters cheap |
...
| App feels slow even when the grid is idle | Grid was re-created repeatedly (`createGrid` called inside a `computed`/`watchEffect` that keeps re-running) | Call `createGrid()`/`useCreateGrid()` once, directly in `<script setup>` — this is the single most common cause of *all* grid performance complaints |
| Selecting one row re-renders the whole page | Usually a symptom of the grid being rebuilt repeatedly (see above), not a real selection issue | Same fix — once the engine is stable across renders, selection updates stay isolated to the affected row |

---

Next: [15 · Full Working Example](/docs/vue/full-example)
