...
If a filter returns zero rows, the grid shows a "No matching results" empty state. You can customize that message — see [Chapter 09 · Appearance](/docs/vue/appearance#empty-states).
...
Combine this with `onFilterChange` (writing the model back to the URL or `localStorage`) for the same "restore on reload" pattern used for sorting in [Chapter 03](/docs/vue/sorting#persisting-sort-across-page-reloads).
...
A text filter, a number filter, and a date filter on three different columns — click a header's funnel icon to open its filter. All three combine with AND, so narrowing one column narrows the visible rows for the others too. Written as `defineComponent` + `h()`, per the note in [Chapter 01](/docs/vue/getting-started#try-it-live).
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Typing feels laggy on a large dataset | `debounceMs` too low, or a heavy `customFilter` running on every keystroke | Raise `debounceMs`, or move filtering to the server with a `dataSource` (see [Chapter 14](/docs/vue/performance#server-side-data-with-datasource)) |
...
Next: [05 · Pagination](/docs/vue/pagination)
