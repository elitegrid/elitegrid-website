...
> `columnId` is the column's `field` (see [Chapter 02](/docs/vue/columns)).
...
| Sort seems to reset when columns are rebuilt | `columns` was recreated as a brand-new array (e.g. inside a `computed` that reruns on unrelated state) — a fresh `comparator` function each time counts as "changed" | Define `columns` once outside reactive tracking, or make sure the `computed` only reruns when the columns genuinely need to change |

---

Next: [04 · Filtering](/docs/vue/filtering)
