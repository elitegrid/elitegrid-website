...
> `columnId` is the column's `field` (see [Chapter 02](/docs/vanilla/columns)).
...
When a column reads its value through `value.getter` (see [Chapter 08](/docs/vanilla/formatting-values#the-value-group--custom-data-access)) instead of a plain `field`, sorting still works the same way — the `comparator` (or the default sort) receives whatever the **getter returns**, not the raw row. That means you can sort by a value that doesn't exist anywhere in your data:
...
toolbar.appendChild(makeBtn('Reset sort', () => api?.clearSort()))
...
| `onSortChange` never fires | Handler was passed to `grid.updateEvents()` on a *different* `grid` object than the one that's mounted | Double-check you're calling `updateEvents` on the same `grid` reference returned by `createGrid()` and passed to `mount()` |

---

Next: [04 · Filtering](/docs/vanilla/filtering)
