...
function killLeftoverProcesses() {
  ...
  try {
    if (isWindows) {
      // On Windows use WMIC to find and kill relevant node processes
      const output = execSync('wmic process where "name=\'node.exe\'" get processid,commandline /format:csv 2>nul', { encoding: 'utf8' });
      ...
      for (const line of lines) {
        ...
        if (!pid || pid === currentPid || pid === process.ppid) continue;

        const isPostCSS = cmd.includes('next/dev/build/postcss.js') || cmd.includes('next\\dev\\build\\postcss.js');
        ...
        if (isPostCSS || isProjectProcess || isNextServerOrWorker) {
          console.log(`[Dev Wrapper] Killing leftover process ${pid}`);
          try {
...
const child = spawn('npx', ['next', 'dev'], {
  cwd: workspaceDir,
  stdio: 'inherit',
  shell: isWindows, // required on Windows so npx.cmd is resolved
});
...
function killChild(signal) {
  ...
  if (child && child.pid) {
    try {
      ...
      if (isWindows) {
        try { execSync(`taskkill /F /T /PID ${child.pid} 2>nul`); } catch (e) {}
      } else {
        child.kill('SIGINT');
...
process.on('SIGINT', () => { killChild('SIGINT'); process.exit(0); });
